...
export default {
	...
	computed: {
		...
		showStartRow() {
			return this.page * this.byPage - (this.byPage - 1);
		},

		showEndRow() {
			let endRow = this.showStartRow + this.byPage - 1;

			return endRow > this.$props.total ? this.$props.total : endRow;
		},

		/** show message with pagination data */
		showPaginationData() {
			return `Showing ${this.showStartRow} to ${this.showEndRow} of ${this.totalRows} rows`;
		},
